...
function Especialidad() {
    ...
    const fnEspecialidad = async () => {    
        const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/especialidad`,{
            'id' : id
        })
    ...
    const fnGuardar = async (e) => {
        e.preventDefault();
          ...
          const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/especialidad/guardar`,{
            'id' : id,
            'nombre' : nombre 
          })
